...
void main_myproject(void)
{
    ...
    // Queues creation:
    EventQueue = xQueueCreate(10000, sizeof(event_t));
    LogQueue = xQueueCreate(10000, sizeof(event_t));
    AmbulanceQueue = xQueueCreate(100, sizeof(event_t));
    PoliceQueue = xQueueCreate(100, sizeof(event_t));
    FDQueue = xQueueCreate(100, sizeof(event_t));
    ...
    if (xTaskCreate(event_gen, "Event Generator", PTHREAD_STACK_MIN,NULL,2,NULL) != pdPASS){
        printf("Error creating 'Event Generator' task");
    }
    if (xTaskCreate(dispatch_q, "Dispath Queue", PTHREAD_STACK_MIN,NULL,3,NULL) != pdPASS){
        printf("Error creating 'Dispath queue' task");
    }
    if (xTaskCreate(logging, "Logging", PTHREAD_STACK_MIN,NULL,1,NULL) != pdPASS){
        printf("Error creating 'Logging' task");
    }

    if (xTaskCreate(ambulance, "Ambulance", PTHREAD_STACK_MIN,NULL,2,&ambulanceTaskHandle) != pdPASS){
        printf("Error creating 'Ambulance' task");
    }
    if (xTaskCreate(police, "Police", PTHREAD_STACK_MIN,NULL,3,&policeTaskHandle) != pdPASS){
        printf("Error creating 'Police' task");
    }
    if (xTaskCreate(fd, "Fire Department", PTHREAD_STACK_MIN,NULL,1,&fdTaskHandle) != pdPASS){
